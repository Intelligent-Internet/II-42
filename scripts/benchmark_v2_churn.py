...
DEFAULT_DB_NAME = 'psql_bm25s_v2_churn_bench'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Run the v2 mixed-churn maintenance benchmark.'
    )
...
def maintenance_policy(cur: psycopg.Cursor, index_name: str) -> str:
    cur.execute(
        '''
        SELECT format(
            'psql_bm25s_maintenance_policy(consistency=%s, '
            'auto_rebuild_threshold=%s, auto_rebuild_delta_bytes=%s, '
            'auto_rebuild_churn_ratio=%s)',
            consistency,
            coalesce(auto_rebuild_threshold::text, 'NULL'),
            coalesce(auto_rebuild_delta_bytes::text, 'NULL'),
            coalesce(auto_rebuild_churn_ratio::text, 'NULL')
        )
        FROM public.psql_bm25s_index_details(%s::regclass)
        ''',
        (index_name,),
...
def setup_table(
    ...
    cur.execute(
        ...
        CREATE INDEX {mode.table_name}_bm25_idx
            ON {mode.table_name} USING psql_bm25s (token_ids)
...
def default_modes() -> list[ModeConfig]:
    return [
        ...
        ModeConfig(
            table_name='docs_auto_threshold_count',
            reloptions=(
                "method = 'lucene', idf_method = 'lucene', "
        ...
        ModeConfig(
            table_name='docs_auto_threshold_tuned',
            reloptions=(
                "method = 'lucene', idf_method = 'lucene', "
                'auto_rebuild_threshold = 1000, '
