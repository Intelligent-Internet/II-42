#!/usr/bin/env python3
"""Render an index-size-vs-dataset-scale SVG from the PG18 matrix JSON."""
...
OUTPUT_SVG = (
    ...
    / 'pg18-index-size-vs-dataset-scale-2026-04-02.svg'
)
...
def load_rows() -> list[dict[str, object]]:
    ...
    for dataset, payload in data['results'].items():
        ...
        for key, _, _, _ in ENGINES:
            row['series'][key] = {
                'index_bytes': float(payload[key]['build_bytes']),
            }
        rows.append(row)
...
def main() -> None:
    ...
    y_min = 10 ** math.floor(math.log10(min_size))
    y_max = 10 ** math.ceil(math.log10(max_size))
    ...
    add(
        svg_text(
            WIDTH / 2,
            42,
            'Index Size vs Dataset Scale (PG18 BEIR Matrix)',
            size=28,
            weight='700',
        )
    )
    add(
        svg_text(
            ...
            'y-axis: PostgreSQL index relation size (log scale)',
            size=15,
            fill='#555',
        )
    )
    ...
    for power in range(int(math.log10(y_min)), int(math.log10(y_max)) + 1):
        ...
        add(svg_text(LEFT - 16, y + 4, human_size(value), size=13,
                     fill='#666', anchor='end'))
    ...
    for key, _label, color, marker in ENGINES:
        ...
        for row in rows:
            ...
            y = TOP + PLOT_H - log_scale(
                row['series'][key]['index_bytes'], y_min, y_max, PLOT_H
            )
            points.append((x, y))
        path = ' '.join(
            [
    ...
    for layout in label_layouts:
        ...
        add(svg_text(label_x, y_label, layout['dataset'], size=13,
                     weight='600'))
        add(svg_text(label_x, y_label + 18, layout['docs_label'], size=12,
                     fill='#666'))
    ...
    add(
        ...
        f'{svg_text(0, 0, "Index size", size=16, weight="600")}'
        f'</g>'
    )

    legend_x = LEFT + PLOT_W - 230
    # Keep the legend in the reserved header band. The size chart has a high
    # right-side series, so placing the legend inside the plot hides that corner.
    legend_y = TOP - 120
    add(
        f'<rect x="{legend_x - 16}" y="{legend_y - 28}" width="242" '
        f'height="140" rx="10" fill="#fffaf0" stroke="#d8cfbf"/>'
    )
    ...
    add(
        svg_text(
            LEFT,
            HEIGHT - 84,
            'Note: size is PostgreSQL index relation bytes recorded as '
            'build_bytes in the PG18 matrix.',
            size=13,
            fill='#666',
            anchor='start',
        )
    ...
    add(
        svg_text(
            LEFT,
            HEIGHT - 58,
            'Representative largest dataset: msmarco | ids 2.98GiB | '
            'text[] 3.00GiB | pg_search 1.65GiB | vchord 12.59GiB',
            size=13,
            fill='#666',
            anchor='start',
        )
