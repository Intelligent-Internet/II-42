#!/usr/bin/env python3
"""Render a QPS-vs-dataset-scale SVG chart from the PG18 matrix JSON."""
...
OUTPUT_SVG = (
    ...
    / 'pg18-qps-vs-dataset-scale-2026-04-02.svg'
)
...
def main() -> None:
    ...
    y_min = 10 ** math.floor(math.log10(min_qps))
    y_max = 10 ** math.ceil(math.log10(max_qps))
    ...
    add(svg_text(WIDTH / 2, 42, 'QPS vs Dataset Scale (PG18 BEIR Matrix)',
                 size=28, weight='700'))
    add(
        svg_text(
            WIDTH / 2,
            68,
            'x-axis: document count (log scale) | y-axis: query throughput '
            'QPS (log scale)',
            size=15,
            fill='#555',
        )
    ...
    for power in range(int(start_power), int(math.log10(x_max)) + 1):
        ...
        add(svg_text(x, TOP + PLOT_H + 24, human_tick(value),
                     size=13, fill='#666'))
    ...
    for power in range(int(math.log10(y_min)), int(math.log10(y_max)) + 1):
        ...
        add(svg_text(LEFT - 16, y + 4, human_count(value),
                     size=13, fill='#666', anchor='end'))
    ...
    for key, label, color, marker in ENGINES:
        points: list[tuple[float, float]] = []
        for row in rows:
            x = LEFT + log_scale(row['documents'], x_min, x_max, PLOT_W)
            y = TOP + PLOT_H - log_scale(
                row['series'][key]['qps'], y_min, y_max, PLOT_H
            )
            points.append((x, y))
        path = ' '.join(
            [
    ...
    # Dataset labels arranged in callout lanes to avoid overlap.
    label_rows = [
        TOP + PLOT_H + 56,
        TOP + PLOT_H + 106,
        TOP + PLOT_H + 156,
    ...
    add(
        ...
        f'{svg_text(0, 0, "Query throughput (QPS)", size=16, weight="600")}'
        f'</g>'
    )
    ...
    add(svg_text(legend_x + 95, legend_y - 8, 'Engines', size=16,
                 weight='700'))
    for idx, (_, label, color, marker) in enumerate(ENGINES):
        y = legend_y + idx * 28
    ...
    add(
        svg_text(
            LEFT,
            HEIGHT - 84,
            'Note: QPS also depends on query count, query length, and token '
            'distribution; scale is the dominant visual guide here.',
            size=13,
            fill='#666',
            anchor='start',
        )
    ...
    add(
        svg_text(
            LEFT,
            HEIGHT - 58,
            'Representative largest dataset: msmarco | Python reference 1.61 | '
            'ids 96.67 | text[] 82.13 | pg_search 4.44 | vchord 18.20',
            size=13,
            fill='#666',
            anchor='start',
        )
