...
> Historical note: the active implementation has moved to staged segmented
> generation publish. See
> [Current Maintenance Lifecycle](../../../maintenance-lifecycle.md)
...
- carry forward segmented delta records that arrive during staging;
- take a short non-blocking swap lock;
- if the index changed in a way that is not a compatible segmented tail, discard the
...
The V1 carry-forward path copies segmented tail records after a staged
...
it preserves segmented tail work that arrived during staging and avoids throwing
